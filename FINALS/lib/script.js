// Get all the images and popup elements
...
// Get the About Me button and add a click event listener
const aboutButton = document.querySelector('.btn:nth-of-type(1)');
aboutButton.addEventListener('click', scrollToAbout);

// Get the My Projects button and add a click event listener
const projectsButton = document.querySelector('.btn:nth-of-type(2)');
projectsButton.addEventListener('click', scrollToProjects);

// Scroll to the About section when the About Me button is clicked
function scrollToAbout(event) {
  event.preventDefault();

  // Get the About section's ID
  const aboutSectionId = '#about';

  // Use smooth scrolling behavior to scroll to the About section
  document.querySelector(aboutSectionId).scrollIntoView({
    behavior: 'smooth'
  });
}

// Scroll to the Gallery section when the My Projects button is clicked
function scrollToProjects(event) {
  event.preventDefault();

  // Get the Gallery section's ID
  const gallerySectionId = '#portfolio';

  // Use smooth scrolling behavior to scroll to the Gallery section
